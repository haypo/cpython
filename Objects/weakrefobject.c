...
weakref_richcompare(PyWeakReference* self, PyWeakReference* other, int op)
{
    ...
    if (Py_IS_NONE(PyWeakref_GET_OBJECT(self))
        || Py_IS_NONE(PyWeakref_GET_OBJECT(other))) {
        int res = (self == other);
        if (op == Py_NE)
            res = !res;
        if (res)
...
{
    if (Py_IS_NONE(PyWeakref_GET_OBJECT(proxy))) {
